...
class InferenceDrawer():
    ...
    def predict(self, batch, batch_index=0, iteration=0, visualize_batch=0, visualize_progress=False, visualize_images=[], visualizer=None, word2index=None, index2word=None, references=[], hypothesis=[]):
        with torch.no_grad():
            ...
            for t in range(max_seq_len):
                ...
                for idxs in word_idxs.tolist():
                    hypothesis.append([idx for idx in idxs
                                       if idx != word2index['<s_start>'] and idx != word2index['<pad>']])

                for utterance_set in current_drawer_utterance.cpu().numpy().tolist():
                    refs = []
                    refs.append([idx for idx in utterance_set
                                 if idx != word2index['<s_start>'] and idx != word2index['<pad>']])
                    references.append(refs)
                
                concatenated_feature_beam = current_drawer_img_feat.expand(
                    self.cfg.teller_beamsize, current_drawer_img_feat.size(1), current_drawer_img_feat.size(2))
                ...
                generated_sentence, alpha = utterance_decoder.caption(concatenated_feature_beam, self.cfg.teller_beamsize, enc_state=enc_state_beam)
                ###################################################################
               ...
               #print("image feature map size is: {}".format(image_feature_map.shape))
                ...
                for word_idx in current_teller_utterance.cpu().numpy().tolist()[0]:
                    if word_idx != word2index['<s_start>'] and word_idx != word2index['<pad>'] and word_idx != word2index['<s_end>']:
                        current_teller_utterance_txt.append(index2word[int(word_idx)])
                    if word_idx == word2index['<s_end>']:
                         break
                ...
                for word_idx in current_drawer_utterance.cpu().numpy().tolist()[0]:
                    if word_idx != word2index['<s_start>'] and word_idx != word2index['<pad>'] and word_idx != word2index['<s_end>']:
                        current_drawer_utterance_txt.append(index2word[word_idx])
                    if word_idx == word2index['<s_end>']:
                         break
                current_drawer_utterance_txt = ' '.join(current_drawer_utterance_txt)

                output_dialog["dialog"].append({"turn": t, "drawer_utt": current_drawer_utterance_txt, "teller_utt": current_teller_utterance_txt, "pred_drawer_utt": ' '.join(sentence_tokens)})
            ...
            #save dialog
            _save_dialog(output_dialog, self.results_path, iteration, scene_id)
    ...
    def load(self, pre_trained_path, iteration=None):
        ...
        #Added the function to load dialog generation modules
        self.drawer_img_encoder.load_state_dict(snapshot['drawer_img_encoder_state_dict'])
        self.dialog_encoder.load_state_dict(snapshot['dialog_encoder_state_dict'])
        self.utterance_decoder.load_state_dict(snapshot['utterance_decoder_state_dict'])
    ...
    def generate_im_utt(self, current_turns_embedding, current_turns_lengths, input_utt, input_utt_len, word2index, index2word, batch_size=1):
        with torch.no_grad():
            #define batch_size
           ...
           #print("image feature map size is: {}".format(image_feature_map.shape))
            ...
            #generate the utt########################################
            current_drawer_img_feat = self.drawer_img_encoder(self.drawer_prev_img)
            current_dialog_hidden, enc_state = self.dialog_encoder(input_utt, input_utt_len, initial_state=self.prev_enc_state)
            self.prev_enc_state = enc_state

            concatenated_feature_beam = current_drawer_img_feat.expand(self.cfg.teller_beamsize, current_drawer_img_feat.size(1), current_drawer_img_feat.size(2))
            if enc_state is not None:
                enc_state_beam = (enc_state[0].expand(self.cfg.teller_beamsize, enc_state[0].size(1), enc_state[0].size(2)),
                                  enc_state[1].expand(self.cfg.teller_beamsize, enc_state[
                                                      1].size(1), enc_state[1].size(2))
...
def _save_predictions(images, text, scene_id, results_path, gt_images, unorm=None, target_im=None, iteration=0, image_gen_mode="real"):
    for i, scene in enumerate(scene_id):
        ...
        cv2.imwrite(os.path.join(results_path, str(scene) + '_gt', 'target.png'),
                    target_image)
def _save_dialog(dialog, output_dir, iteration, batch_index):
    """
    Save the dialog to the output_dir
    """
    output_file_path = output_dir + '/' + str(batch_index[0]) + '/' + 'drawer_dialog_{}.json'.format(iteration)
    with open(output_file_path, 'w') as fp:
        json.dump(dialog, fp, indent=4)
