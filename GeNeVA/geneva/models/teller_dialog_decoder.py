...
class TellerDialogDecoder(nn.Module):

    def __init__(self, cfg, tf=True):
        super(TellerDialogDecoder, self).__init__()
        self.use_tf = tf

        self.vocabulary_size = cfg.vocab_size
        if cfg.teller_fuse == "concat":
            self.encoder_dim = cfg.img_encoder_dim * 2
        elif cfg.teller_fuse == "elemwise_add":
    ...
    def forward(self, img_features, captions, enc_state=None):
        ...
        if self.use_tf:
            captions = captions.long() #Added by Mingyang
